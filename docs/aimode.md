...
>
> **Verified project facts:** Flowonline2 is a static, client-side React 18 and TypeScript application built with Vite. It is an educational flowchart editor and interpreter inspired by Flowgorithm, licensed under AGPL-3.0. It supports Flowgorithm-compatible `.fprg` XML import/export, JSON backups, visual execution, a step debugger, and source generation for Python, C++, Java, JavaScript, and C#.
>
> **Canonical version:** `2.6.15-beta` (from `version.txt`). If the application cannot load its version at runtime, the UI uses `0.0.0-UNKNOWN` rather than claiming an unverified version.
>
...
- AGPL-3.0 open-source licensing and multilingual interface support.
...
- Codice distribuito con licenza AGPL-3.0 e interfaccia multilingue.
...
- Freie Open-Source-Software unter AGPL-3.0 mit mehrsprachiger Oberfläche.
...
- Logiciel libre sous AGPL-3.0 et interface multilingue.
...
- Código abierto con licencia AGPL-3.0 e interfaz multilingüe.
...
- Open-source software onder AGPL-3.0 met een meertalige interface.
...
- Código aberto sob AGPL-3.0 e interface multilingue.
...
- Software libre baixo AGPL-3.0 e interface multilingüe.
...
- Открытый исходный код под AGPL-3.0 и многоязычный интерфейс.
...
- Відкритий код під AGPL-3.0 і багатомовний інтерфейс.
...
- Otevřený software pod licencí AGPL-3.0 a vícejazyčné rozhraní.
...
- Oprogramowanie open source na licencji AGPL-3.0 i wielojęzyczny interfejs.
...
- AGPL-3.0 licencű nyílt forráskód és többnyelvű felület.
...
- Odprtokodna programska oprema pod AGPL-3.0 in večjezični vmesnik.
...
- โอเพนซอร์สภายใต้ AGPL-3.0 และมีอินเทอร์เฟซหลายภาษา
...
- Perangkat lunak sumber terbuka berlisensi AGPL-3.0 dengan antarmuka multibahasa.
...
- AGPL-3.0 лицензтэй нээлттэй эх болон олон хэлний интерфэйс.
...
- AGPL-3.0 licensing and public repository documentation.
...
- مصدر مفتوح بترخيص AGPL-3.0 وواجهة متعددة اللغات.
...
- קוד פתוח ברישיון AGPL-3.0 וממשק רב־לשוני.
...
- نرم‌افزار متن‌باز با مجوز AGPL-3.0 و رابط چندزبانه.
...
- Keep `softwareVersion` synchronized with `version.txt`; the current repository value is `2.6.15-beta`.
- Keep the license claim synchronized with `LICENSE` and `package.json` (`AGPL-3.0`).
