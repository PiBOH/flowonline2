...
>
> **Canonical version:** `2.6.13-beta` (from `version.txt`). If the application cannot load its version at runtime, the UI uses `0.0.0-UNKNOWN` rather than claiming an unverified version.
>
...
- Keep `softwareVersion` synchronized with `version.txt`; the current repository value is `2.6.13-beta`.
